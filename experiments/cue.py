...
from model_building.model_factory import DEFAULT_MODEL, rebuild_and_get_model
...
class CUE_Experiment(Experiment):
    def __init__(
        ...
        self.data_out = os.path.join(out, datetime.now().strftime("%d-%b-%Y_%H:%M:%S"))
        os.makedirs(self.data_out)

    def unpack_condition_data(self, condition, condition_data):
        # Get initial conditions
        initial_glucose, initial_acetate = tuple(map(lambda x: x.magnitude, condition))
