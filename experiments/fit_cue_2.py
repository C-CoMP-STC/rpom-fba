...
def main():
    ...
    for k, data in condition_data.items():
        ...
        def sse(t, b, g, a):
            ...
            return (
                np.nan_to_num((np.log(b_hat) - np.log(b_true)) ** 2).sum()
                + np.nan_to_num((np.log(g_hat) - np.log(g_true)) ** 2).sum()
                + np.nan_to_num((np.log(a_hat) - np.log(a_true)) ** 2).sum()
            )
        ...
        step_schedule = np.logspace(-3, -4, num=len(dt_schedule) * MAXITER)
